...
| Hecho (2026-09-23) | Historial de git reescrito (NIF y dirección → `[NIF eliminado]` / `[dirección eliminada]` en las versiones antiguas de los textos legales; force push con lease; 0 objetos con datos en GitHub `main` y en la copia local; identificadores de commit actualizados en `plans/`). **Riesgo residual aceptado por el titular (opción C, 2026-09-23)**: no pedirá la purga al soporte de GitHub; los commits antiguos siguen accesibles por su identificador desde la página de actividad del repo (force push `9825d18`→`4978e43`). Comprobado fuera de GitHub: Software Heritage no tiene el repo; Wayback Machine solo tiene la portada de 2025, sin los datos. Alternativas descartadas: recrear el repo (gratis, definitiva) o GitHub Pro + repo privado. La copia de seguridad del historial original (`_copias/…bundle`) la borró el titular (2026-09-23) | 018 |
| Hecho (2026-09-23) | La web antigua `magnusmcmdev.github.io` (export de WordPress) seguía publicada con el NIF y la dirección en 12 páginas: el titular **borró el repositorio** `MagnusMCMDev/magnusmcmdev.github.io`. Comprobado: todas sus páginas dan 404 sin datos; vorama.es, la redirección `magnusmcmdev.github.io/vorama-astro/` y `Certificate_Asistant` siguen funcionando. El titular borró también la copia local (2026-09-23) | 018 |
