...
| Tras desplegar 014 | Search Console: `sitemap-index.xml` enviado y `page-sitemap.xml` de WordPress eliminado por el titular (2026-09-23); el sitio sirve bien el sitemap (200 `application/xml`, también a Googlebot; 15 URLs en `sitemap-0.xml`). Pendiente: que pase a "Correcto" y la lista de 404 → añadir cada URL antigua a `redirects` | 014 |
